	...
	void *pdata)
{
	...
	param_info.id = SYSCALL_CREATE_PERIODIC_TASK;
	...
	arg[4] = stack_size_in_bytes;
	arg[5] = (UINT32)task_name;
	arg[6] = (UINT32)periodic_entry_function;
	arg[7] = (UINT32)pdata;
	
	result = _OS_Syscall(&param_info, &arg, &ret, NULL);
	
	// Store the return value
	*task = (OS_PeriodicTask) ret[0];
